 ...
 */
export default class SqlBlobHandler extends SqlBaseHandler
  implements IBlobHandler {
  /**
  ...
  constructor(
    ...
    sqlDataStore: IBlobMetadataStore,
    extentStore: IExtentStore,
    logger: ILogger,
    private readonly rangesManager: IPageBlobRangesManager
  ) {
    super(dataStore, sqlDataStore, extentStore, logger);
  }
  ...
  public async download(
    ...
    if (blob.snapshot === "") {
      SqlBlobHandler.checkLeaseOnReadBlob(
        context,
        blob,
  ...
  public async getProperties(
    ...
    const blob = await this.sqlDataStore.getBlobProperties(
      accountName,
      containerName,
      blobName
    );
  ...
  public async delete(
    ...
    // Check Lease status
    if (againstBaseBlob) {
      SqlBlobHandler.checkBlobLeaseOnWriteBlob(
        context,
        blob,
        options.leaseAccessConditions
      );
    ...
    let blobs = [];
    do {
      [blobs, marker] = await this.sqlDataStore.listBlobs(
        blob.accountName,
        blob.containerName,
        blob.name,
        undefined,
  ...
  public async setHTTPHeaders(
    ...
    blob = SqlBlobHandler.UpdateBlobLeaseStateOnWriteBlob(blob);
  ...
  public async startCopyFromURL(
    ...
    // TODO: Check dest Lease status, and set to available if it's expired, see sample in SqlBlobHandler.setMetadata()
  ...
  public async setTier(
    ...
    SqlBlobHandler.checkBlobLeaseOnWriteBlob(context, blob, undefined);
    ...
    //     throw Error(
    //       `SqlBlobHandler:setTier() PageBlobAccessTierThreshold doesn't define tier threshold for tier ${tier}`
    //     );
    //   }
  ...
  private async downloadBlockBlob(
    ...
    this.logger.info(
      // tslint:disable-next-line:max-line-length
      `SqlBlobHandler:downloadBlockBlob() NormalizedDownloadRange=bytes=${rangeStart}-${rangeEnd} RequiredContentLength=${contentLength}`,
      context.contextID
    );
  ...
  private async downloadPageBlob(
    ...
    this.logger.info(
      // tslint:disable-next-line:max-line-length
      `SqlBlobHandler:downloadPageBlob() NormalizedDownloadRange=bytes=${rangeStart}-${rangeEnd} RequiredContentLength=${contentLength}`,
      context.contextID
    );
  ...
  private async getSimpleBlobFromStorage(
    ...
    blob = SqlBlobHandler.updateLeaseAttributes(blob, blobCtx.startTime!);
